(() => {
  const stage = document.getElementById("clay");
  const canvas = document.getElementById("clay-canvas");
  const fallback = document.getElementById("clay-reveal");
  const fallbackPath = document.getElementById("clay-brush-path");
  const fallbackMask = document.getElementById("clay-brush-mask");
  const fallbackImage = document.getElementById("clay-image");
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
  const BASE_COLOR = [247 / 255, 37 / 255, 38 / 255];
  ...
  const reliefSource = `
    ...
    void main() {
      ...
      // The relief's own shading drives a small rise as it emerges from the plane.
      ...
      imageUV += normal * 0.025 * amount * (1.0 - amount) * motion;
      vec3 relief = texture2D(artwork, imageUV).rgb;
      float detail = abs(light(relief) - (left + right + down + up) * 0.25);
  ...
  function initialize() {
    ...
    try {
      programs = {
        ...
        relief: program(reliefSource, ["field", "artwork", "cover", "imageTexel", "baseColor", "motion"])
      };
      ...
      console.warn("Clay reveal uses the soft SVG fallback:", error.message);
      showFallback();
    }
  }
  function reset() {
  ...
  function drawField(start, end, paint, dt, now) {
    ...
    gl.uniform3fv(relief.uniforms.baseColor, BASE_COLOR);
  ...
  stage.addEventListener("pointermove", (event) => {
    ...
    if (!follower || now - lastInput > TRAIL_LIFETIME) follower = { ...pointer };
    lastInput = now;
    if (!animationFrame) {
      lastFrame = now - 16;
      animationFrame = requestAnimationFrame(render);
