// Native WebGL renderer for the studio's white cursor ribbon.
...
window.createCursorRenderer = (canvas, fallback) => {
  ...
  let height = 1;
  let pixelRatio = 1;
  const coordinates = new Float32Array(31 * 3);
  const vertexSource = `
  ...
  function draw(points, opacity) {
    ...
    points.forEach((point, index) => {
      coordinates.set([point.x, point.y, point.radius], index * 3);
    ...
    gl.uniform3fv(locations["points[0]"], coordinates);
