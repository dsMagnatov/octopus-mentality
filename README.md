...
The ribbon uses native WebGL for smooth, soft edges, with an SVG alpha mask revealing the ornament. It falls back to a white SVG path if WebGL is unavailable. Animation stops completely once the ribbon fades. Its width and timing can be adjusted through the constants at the top of `app.js`.
